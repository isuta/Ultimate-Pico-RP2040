...
角度制御型サーボ（'position'）の場合、以下の追加設定が利用可能です：
...
**パルス幅について:**
- 標準的なサーボは1000-2000μsですが、SG90互換は500-2500μsで180度の可動範囲を確保できることが多いです
- サーボの仕様書を確認して適切な値に調整してください

#### 個体差キャリブレーション

サーボモーターには個体差があるため、サーボごとに微調整が可能です：

```python
# config.py
SERVO_POSITION_CALIB = {
    # サーボインデックス: {調整パラメータ}
    1: {'min_pulse': 600, 'max_pulse': 2400, 'offset_deg': 5}
}
```
...
| パラメータ | 単位 | 説明 | 使用例 |
|-----------|------|------|--------|
| `min_pulse` | μs | 0度の時のパルス幅を上書き | 0度が行き過ぎる場合に値を増やす |
| `max_pulse` | μs | 180度の時のパルス幅を上書き | 180度が届かない場合に値を増やす |
| `offset_deg` | 度 | 全体の角度オフセット | 全体的にズレている場合に補正 |

**調整例:**

#### 例1: 0度が行き過ぎる場合（-10度に見える）
```python
SERVO_POSITION_CALIB = {
    1: {'min_pulse': 610}  # 約10度分（1度≈11μs）引き上げ
}
```

#### 例2: 180度が届かない場合（170度で止まる）
```python
SERVO_POSITION_CALIB = {
    1: {'max_pulse': 2600}  # パルス幅を増やして届かせる
}
```
...
```python
SERVO_POSITION_CALIB = {
    1: {'min_pulse': 610},           # サーボ#1は0度を調整
    2: {'offset_deg': -5},           # サーボ#2は全体を-5度シフト
    0: {'max_pulse': 2550}           # サーボ#0は180度を調整
}
```

**調整の手順:**

1. `servo_calibrate`シナリオで0/45/90/135/180度を確認
2. 問題のある端点（0度または180度）を確認
3. 必要に応じて`min_pulse`または`max_pulse`を10-20μs刻みで調整
4. 再度シナリオで動作確認
5. 理想的な位置になるまで繰り返し

**補足:**
- 変更不要なパラメータは記載不要（デフォルト値が使われます）
- サーボホーンの取り付け位置を機械的に調整することでも改善できます
- 供給電圧が不安定だと端点がズレることがあります
...
```python
# config.py
REPEAT_MAX_DEPTH = 3  # repeatコマンドの最大ネスト深度
```

**説明:**
`repeat`コマンドをネスト（入れ子）にできる最大の深さを指定します。メモリ消費とスタックオーバーフローを防ぐための制限です。

**カスタマイズ:**
- **シンプルなシナリオ**: 2（ネストをほぼ使わない場合）
- **複雑なシナリオ**: 4-5（メモリに余裕がある場合）
- **デフォルト**: 3（バランス良好）

**動作:**
- 深度を超過すると、エラーメッセージを出力してシナリオを停止
- 無限ループ防止のため、深すぎるネストは推奨しません
